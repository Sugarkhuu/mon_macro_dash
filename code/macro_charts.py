...
list_im_auto = ['im_auto_vol','im_auto']

for elem in list_im_auto:
    dm[elem] = uncumul(dm[elem + "_cum"])

dq['hh_loan'] = convert(dm['hh_loan'].copy(),'q')
dq['cpi'] = convert(dm['cpi'].copy(),'q')

dq['hh_inc_real'] = dq['hh_inc']/dq['cpi']/1e3
dq['hh_exp_real'] = dq['hh_exp']/dq['cpi']/1e3
dq['wage_avg_real'] = dq['wage_avg']/dq['cpi']
dq['wage_median_real'] = dq['wage_median']/dq['cpi']
...
plot_section("QUICK INDICATORS TO TRACK")

plt.figure()
plt.subplot(gs22[0,0])
doPlot(line1 = dm['ex_coal_vol_m'],
       title = 'Coal export volume (thou. tn), last: ' + get(dm.ex_coal_vol_m,'last'),
       rng   = [firstM])
plt.subplot(gs22[0,1])
doPlot(line1 = dm['ex_coal_m']/1e3,
       title = 'Coal export revenue (thou. USD), last: ' + get(dm['ex_coal_m'],'last'),
       rng   = [firstM])
plt.subplot(gs22[1,0])
doPlot(line1 = dm['ex_coal_p'],
       title = 'Coal export price (USD), last: ' + get(dm['ex_coal_p'],'last'),
       rng   = [firstM])
...
plt.figure()
plt.subplot(gs22[0,0])
doPlot(line1 = dm['ex_copper_vol_m'],
       title = 'Copper export volume (thou. tn), last: ' + get(dm.ex_copper_vol_m,'last'),
       rng   = [firstM])
plt.subplot(gs22[0,1])
doPlot(line1 = dm['ex_copper_m']/1e3,
       title = 'Copper export revenue (thou. USD), last: ' + get(dm['ex_copper_m'],'last'),
       rng   = [firstM])
plt.subplot(gs22[1,0])
doPlot(line1 = dm['ex_copper_p'],
       title = 'Copper export price (USD), last: ' + get(dm['ex_copper_p'],'last'),
       rng   = [firstM])
plt.xticks(fontsize=5)
plt.subplots_adjust(hspace=0.5)
...
plt.figure()
plt.subplot(gs22[0,0])
doPlot(line1 = dq['hh_inc']/1e3,
       line2 = dq['wage_avg'],
       line3 = dq['wage_median'],
       title = 'Household earnings (thou. MNT), last: ' + get(dq.hh_inc,'last'),
       leg = ['Household income','Salary (average)','Salary (median)'],
       rng   = [firstQ])
plt.subplot(gs22[0,1])
doPlot(line1 = pct(dq['hh_inc'],-4),
       line2 = pct(dq['wage_avg'],-4),
       line3 = pct(dq['wage_median'],-4),
       title = 'Household earnings (%, YoY), last: ' + get(dq.hh_inc,'last'),
       leg = ['Household income','Salary (average)','Salary (median)'],
       rng   = [firstQ])
plt.subplot(gs22[1,0])
doPlot(line1 = dq['hh_inc']/1e3,
       line2 = dq['hh_exp']/1e3,
       title = 'Household budget (thou. MNT), last: ' + get(dq.hh_inc,'last'),
       leg = ['Income','Expense'],
       rng   = [firstQ])
plt.subplot(gs22[1,1])
doPlot(line1 = pct(dq['hh_inc'],-4),
       line2 = pct(dq['hh_exp'],-4),
       title = 'Household budget (%, YoY), last: ' + get(dq.hh_inc,'last'),
       leg = ['Income','Expense'],
       rng   = [firstQ])
plt.xticks(fontsize=5)
plt.subplots_adjust(hspace=0.5)
...
plt.figure()
plt.subplot(gs22[0,0])
doPlot(line1 = dq['hh_inc_real'],
       line2 = dq['wage_avg_real'],
       line3 = dq['wage_median_real'],
       title = 'Household real earnings (scaled by CPI), last: ' + get(dq.hh_inc,'last'),
       leg = ['Household income','Salary (average)','Salary (median)'],
       rng   = [firstQ])
plt.subplot(gs22[0,1])
doPlot(line1 = pct(dq['hh_inc_real'],-4),
       line2 = pct(dq['wage_avg_real'],-4),
       line3 = pct(dq['wage_median_real'],-4),
       title = 'Household real earnings (%, YoY), last: ' + get(dq.hh_inc,'last'),
       leg = ['Household income','Salary (average)','Salary (median)'],
       rng   = [firstQ])
plt.subplot(gs22[1,0])
doPlot(line1 = dq['hh_inc_real'],
       line2 = dq['hh_exp_real'],
       title = 'Household real budget (scaled by CPI), last: ' + get(dq.hh_inc,'last'),
       leg = ['Income','Expense'],
       rng   = [firstQ])
plt.subplot(gs22[1,1])
doPlot(line1 = pct(dq['hh_inc_real'],-4),
       line2 = pct(dq['hh_exp_real'],-4),
       title = 'Household real budget (%, YoY), last: ' + get(dq.hh_inc,'last'),
       leg = ['Income','Expense'],
       rng   = [firstQ])
plt.xticks(fontsize=5)
plt.subplots_adjust(hspace=0.5)
...
plt.figure()
plt.subplot(gs22[0,0])
doPlot(line1 = dm['hh_loan']/1e6,
       title = 'Consumer loan (trillion MNT), last: ' + get(dm['hh_loan'],'last'),
       rng   = [firstM])
plt.subplot(gs22[0,1])
doPlot(line1 = pct(dm['hh_loan'],-12),
       title = 'Consumer loan (%, YoY), last: ' + get(dm['hh_loan'],'last'),
       rng   = [firstM])
plt.subplot(gs22[1,0])
doPlot(line1 = pct(dq['hh_inc'],-4),
       line2 = pct(dq['hh_exp'],-4),
       line3 = pct(dq['hh_loan'],-4),
       title = 'Household budget and Consumer loan (%, YoY), last: ' + get(dq['hh_loan'],'last'),
       leg = ['Income','Expense','Loan'],
       rng   = [firstM])
plt.subplots_adjust(hspace=0.5)
...
plt.figure()
plt.subplot(gs21[0])
doPlot(line1 = dm['im_auto_vol']/1e3,
       line2 = dm['im_auto_vol'].rolling(window=12).mean()/1e3,
       title = 'Imported automobile number (thou.), last: ' + get(dm['im_auto_vol'],'last'),
       leg   = ['Automobile','12m average'],
       rng   = [firstM])
plt.subplot(gs21[1])
doPlot(line1 = dm['im_auto']/1e3,
       line2 = dm['im_auto'].rolling(window=12).mean()/1e3,
       title = 'Imported automobile (thou. USD), last: ' + get(dm['im_auto'],'last'),
       leg   = ['Automobile','12m average'],
       rng   = [firstM])
plt.subplots_adjust(hspace=0.5)
...
plt.figure()
plt.subplot(gs22[0,0])
doPlot(line1 = dm['price_fl_up'],
       line2 = dm['price_bread'],
       line3 = dm['price_milk_box'],
       title = 'Flour, bread and milk prices, last: ' + get(dm['price_fl_up'],'last'),
       leg   = ['Flour','Bread','Milk'],
       rng = [firstM])
plt.subplot(gs22[0,1])
doPlot(line1 = dm['price_mutton_bone'],
       title = 'Meat price (mutton), last: ' + get(dm['price_mutton_bone'],'last'),
       rng = [firstM])
plt.subplot(gs22[1,0])
doPlot(line1 = dm['price_potato'],
       line2 = dm['price_egg'],
       line3 = dm['price_apple'],
       title = 'Other food price, last: ' + get(dm['price_potato'],'last'),
       leg   = ['Potato','Egg','Apple'],
       rng = [firstM])
plt.xticks(fontsize=5)
plt.subplot(gs22[1,1])
doPlot(line1 = dm['price_petrol_92'],
       line2 = dm['price_diesel'],
       title = 'Petroleum price, last: ' + get(dm['price_petrol_92'],'last'),
       leg   = ['AI92','Diesel'],
       rng = [firstM])
plt.xticks(fontsize=5)
plt.subplots_adjust(hspace=0.5)
...
plt.figure()
plt.subplot(gs22[0,0])
doPlot(line1 = pct(dm['price_fl_up'],-12),
       line2 = pct(dm['price_bread'],-12),
       line3 = pct(dm['price_milk_box'],-12),
       title = 'Flour, bread and milk prices, last: ' + get(dm['price_fl_up'],'last'),
       leg   = ['Flour','Bread','Milk'],
       rng = [firstM])
plt.subplot(gs22[0,1])
doPlot(line1 = pct(dm['price_mutton_bone'],-12),
       title = 'Meat price (mutton, %, YoY), last: ' + get(dm['price_mutton_bone'],'last'),
       rng = [firstM])
plt.subplot(gs22[1,0])
doPlot(line1 = pct(dm['price_potato'],-12),
       line2 = pct(dm['price_egg'],-12),
       line3 = pct(dm['price_apple'],-12),
       title = 'Other food price (%, YoY), last: ' + get(dm['price_potato'],'last'),
       leg   = ['Potato','Egg','Apple'],
       rng = [firstM])
plt.xticks(fontsize=5)
plt.subplot(gs22[1,1])
doPlot(line1 = pct(dm['price_petrol_92'],-12),
       line2 = pct(dm['price_diesel'],-12),
       title = 'Petroleum price (%, YoY), last: ' + get(dm['price_petrol_92'],'last'),
       leg   = ['AI92','Diesel'],
       rng = [firstM])
plt.xticks(fontsize=5)
plt.subplots_adjust(hspace=0.5)
...
doPlot(line1 = dm['rate_depo_fx'],
       ...
       leg   = ['Depo.','Depo. (new)','Lend','Lend (new)','BoM'],
       rng   = [firstM])
plt.subplot(gs21[0])
doPlot(line1 = dm['rate_depo_mnt'],
       ...
       line5 = dm['rate_pol'],
       title = 'Төгрөгийн хүү (%, p.a), last: ' + get(dm['rate_depo_mnt'],'last'),
       leg   = ['Хад.','Хад. (шинэ)','Зээл','Зээл (шинэ)','Бодлогын хүү'],
       rng   = [firstM])
plt.subplots_adjust(hspace=0.5)
...
plt.figure()
doPlot(line1 = dm["ms_ca_fx"]/(dm["ms_ca_mnt"] + dm["ms_ca_fx"]),
       line2 = dm["ms_dep_fx"]/(dm["ms_dep_mnt"] + dm["ms_dep_fx"]),
       line3 = (dm["ms_ca_fx"] + dm["ms_dep_fx"])/(dm["ms_ca_mnt"] + dm["ms_ca_fx"] + dm["ms_dep_mnt"] + dm["ms_dep_fx"]),
       title = 'Dollarization, last: ' + get(dm["ms_ca_mnt"],'last'),
       leg = ['Current account', 'Deposit', 'Total'],
       rng   = [firstM])
plt.subplots_adjust(hspace=0.5)
...
plt.figure()
plt.subplot(gs21[0])
doPlot(line1 = dm['transport_rail'],
       title = 'Railway transport (tonnes), last: ' + get(dm['transport_rail'],'last'),
       rng   = [firstM])
plt.subplot(gs21[1])
doPlot(line1 = dm['transport_rail_rev'],
       title = 'Railway transport revenue (tonnes), last: ' + get(dm['transport_rail_rev'],'last'),
       rng   = [firstM])
plt.subplots_adjust(hspace=0.5)

plt.figure()
plt.subplot(gs21[0])
doPlot(line1 = dm['transport_air'],
       title = 'Air transport (tonnes), last: ' + get(dm['transport_air'],'last'),
       rng   = [firstM])
plt.subplot(gs21[1])
doPlot(line1 = dm['transport_air_rev'],
       title = 'Air transport revenue (tonnes), last: ' + get(dm['transport_air_rev'],'last'),
       rng   = [firstM])
plt.subplots_adjust(hspace=0.5)

plt.figure()
plt.subplot(gs21[0])
doPlot(line1 = dq['transport_auto'],
       title = 'Auto transport (tonnes), last: ' + get(dq['transport_auto'],'last'),
       rng   = [firstQ])
plt.subplot(gs21[1])
doPlot(line1 = dq['transport_auto_rev'],
       title = 'Auto transport revenue (tonnes), last: ' + get(dq['transport_auto_rev'],'last'),
       rng   = [firstQ])
plt.subplots_adjust(hspace=0.5)
...
doPlot(line1 = pct(dm['cpi'],-12),
       line2 = dm['rate_pol'],
       title = 'Инфляци ба бодлогын хүү, last: ' + get(dm['ms_m2'],'last'),
       leg   = ['Инфляци (%, жилийн)','Бодлогын хүү'],
       rng   = [firstM])
plt.subplots_adjust(hspace=0.5)
...
doPlot(line1 = dm['bank_ass_credit_npl_mnt_corp']/dm['bank_ass_credit_mnt_corp']*100, 
       title = 'Corporate MNT NPL ratio, last: ' + get(dm['bank_ass_credit_corp'],'last'),
       rng   = [firstM])
plt.subplot(gs22[1,0])
...
doPlot(line1 = dm['bank_ass_credit_npl_mnt_ind']/dm['bank_ass_credit_mnt_ind']*100, 
       title = 'Individual MNT NPL ratio, last: ' + get(dm['bank_ass_credit_corp'],'last'),
       rng   = [firstM])
plt.subplot(gs22[1,0])
...
pdf = matplotlib.backends.backend_pdf.PdfPages(r"output\Mongolia_data_report_" + name_date + ".pdf")
